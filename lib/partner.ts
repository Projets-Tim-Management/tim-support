import { lexicalToHtml, mediaUrl } from "./content";
...
export async function submitMission(args: {
  ...
  for (const f of files) {
    ...
    if (!/^image\/(jpeg|png|gif|webp)$/.test(f.type)) continue;
    const buffer = Buffer.from(await f.arrayBuffer());
    const media = await payload.create({
      collection: "media",
      data: { alt: f.name },
      file: { data: buffer, mimetype: f.type, name: f.name, size: buffer.length },
    });
  ...
  return { status: 200, data: { success: true, submission_number: number } };
}
