...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  ALTER TABLE "sequences_loss_reasons" ALTER COLUMN "value" SET DATA TYPE text;
  DROP TYPE "public"."enum_sequences_loss_reasons";
  CREATE TYPE "public"."enum_sequences_loss_reasons" AS ENUM('prix', 'fonctionnalites', 'concurrent', 'budget', 'cessation', 'autre', 'sans-reponse', 'pas-le-moment', 'besoin-different', 'solution-interne', 'test-non-concluant', 'peu-utilise', 'complexite', 'support', 'reorganisation');
  ALTER TABLE "sequences_loss_reasons" ALTER COLUMN "value" SET DATA TYPE "public"."enum_sequences_loss_reasons" USING "value"::"public"."enum_sequences_loss_reasons";`)
}
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  ALTER TYPE "public"."enum_sequences_loss_reasons" ADD VALUE 'a-qualifier' BEFORE 'peu-utilise';`)
}
